    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if !is_paused {
                ...
                let mut batch_size = if is_headless { 0 } else { if !is_worker_active { ticks_per_loop * 2 } else { ticks_per_loop } };

                if is_headless {
                    if !is_worker_active {
                        batch_size = ticks_per_loop * 10; 
                ...
                let mut spawned_telemetry = false;
                if let Ok(mut data) = sim_thread_data.try_lock() {
                    if !is_worker_active || !is_headless {
                    ...
                    if survivors_count < data.config.sim.founder_count as usize {
                        ...
                        data.sim = crate::simulation::SimulationManager::new(data.config.world.map_width, data.config.world.map_height, rand::Rng::r#gen(&mut rng), data.config.sim.agent_count, &data.config, survivors);
